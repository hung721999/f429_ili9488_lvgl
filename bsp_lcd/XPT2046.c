...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi) {
	...
	if (hspi->Instance == SPI1) {
		...
		/**SPI1 GPIO Configuration
		 ...
		 PA15     ------> SPI1_NSS
		 */
		GPIO_InitStruct.Pin = XPT2046_SPI1_SCK_Pin | XPT2046_SPI1_MISO_Pin
				| XPT2046_SPI1_MOSI_Pin;
...
void xpt2046_init(void) {
	...
	xpt2046_cs(0);
	xpt2046_xchg_value(0x80);
	xpt2046_xchg_value(0);
	xpt2046_xchg_value(0);
	xpt2046_cs(1);
}
...
void xpt2046_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
	...
	uint8_t irq = xpt2046_irq_pin_read();

	if (irq == 0) {
		xpt2046_cs(0);

		xpt2046_xchg_value(CMD_X_READ); /*Start x read*/

		buf = xpt2046_xchg_value(0); /*Read x MSB*/
		x = buf << 8;
		buf = xpt2046_xchg_value(CMD_Y_READ); /*Until x LSB converted y command can be sent*/
		x += buf;

		buf = xpt2046_xchg_value(0); /*Read y MSB*/
		y = buf << 8;

		buf = xpt2046_xchg_value(0); /*Read y LSB*/
		y += buf;
