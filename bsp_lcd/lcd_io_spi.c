...
void LCD_IO_WriteMultiData8(uint8_t *pData, uint32_t Size, uint32_t dinc)
{
	...
	if (!dinc)
	{
		d8s = *pData;
		pData = &d8s;
		dmacr = DMAX_STREAMX(LCD_DMA_TX)->CR = DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) | (0 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) | (DMACHN(LCD_DMA_TX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_TX) << DMA_SxCR_PL_Pos);
	}
	else
		dmacr = DMAX_STREAMX(LCD_DMA_TX)->CR = DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) | (1 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) | (DMACHN(LCD_DMA_TX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_TX) << DMA_SxCR_PL_Pos);
...
void LCD_IO_WriteMultiData16(uint16_t *pData, uint32_t Size, uint32_t dinc)
{
	...
	if (!dinc)
	{
		d16s = *pData;
		pData = &d16s;
		dmacr = DMA_SxCR_TCIE | (1 << DMA_SxCR_MSIZE_Pos) | (1 << DMA_SxCR_PSIZE_Pos) | (0 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) | (DMACHN(LCD_DMA_TX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_TX) << DMA_SxCR_PL_Pos);
	}
	else
		dmacr = DMA_SxCR_TCIE | (1 << DMA_SxCR_MSIZE_Pos) | (1 << DMA_SxCR_PSIZE_Pos) | (1 << DMA_SxCR_MINC_Pos) | (1 << DMA_SxCR_DIR_Pos) | (DMACHN(LCD_DMA_TX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_TX) << DMA_SxCR_PL_Pos);
...
{
	uint32_t dmacr;
	dmacr = DMA_SxCR_TCIE | (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) | DMA_SxCR_MINC | (DMACHN(LCD_DMA_RX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_RX) << DMA_SxCR_PL_Pos) | (0 << DMA_SxCR_MBURST_Pos);
...
{
	uint32_t dmacr;
	dmacr = DMA_SxCR_TCIE | (1 << DMA_SxCR_MSIZE_Pos) | (1 << DMA_SxCR_PSIZE_Pos) | DMA_SxCR_MINC | (DMACHN(LCD_DMA_RX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_RX) << DMA_SxCR_PL_Pos) | (0 << DMA_SxCR_MBURST_Pos);
...
void LCD_IO_ReadMultiData16to24(uint16_t *pData, uint32_t Size)
{
	...
	DMAX_STREAMX(LCD_DMA_RX)->CR = (0 << DMA_SxCR_MSIZE_Pos) | (0 << DMA_SxCR_PSIZE_Pos) | DMA_SxCR_MINC | (0 << DMA_SxCR_MBURST_Pos) | (DMACHN(LCD_DMA_RX) << DMA_SxCR_CHSEL_Pos) | (DMAPRIORITY(LCD_DMA_RX) << DMA_SxCR_PL_Pos) | DMA_SxCR_CIRC;
	...
	while (BITBAND_ACCESS(SPIX->SR, SPI_SR_RXNE_Pos))
		d8 = SPIX->DR;
	SPIX->CR1 = (SPIX->CR1 & ~SPI_CR1_BR) | ((LCD_SPI_SPD_WRITE << SPI_CR1_BR_Pos) | SPI_CR1_BIDIOE) | SPI_CR1_SPE;
	LCD_IO_Delay(2 ^ LCD_SPI_SPD_READ);
	while (BITBAND_ACCESS(SPIX->SR, SPI_SR_RXNE_Pos))
		d8 = SPIX->DR;
	DMAX_STREAMX(LCD_DMA_RX)->CR = 0;
...
#define DMA1_CLOCK_RX 0
	...
	RCC->AHB1ENR |=
		GPIOX_CLOCK(
			LCD_RS) |
		GPIOX_CLOCK(LCD_CS) | GPIOX_CLOCK(LCD_SCK) | GPIOX_CLOCK(LCD_MOSI) |
